...
def main():
    ...
    for i in range(cfg["net"]["num_classes"]):
        latent_feature = torch.zeros((1, 32)).cuda(model.device_ids[0])
        deterministic_feature = torch.zeros((1, 32)).cuda(model.device_ids[0])
        latent_memory.append(latent_feature)
        deterministic_memory.append(deterministic_feature)
    ...
    for epoch in range(last_epoch, cfg_trainer["epochs"]):
        ...
        if cfg_trainer["eval_on"]:
            ...
            if epoch < cfg["trainer"].get("sup_only_epoch", 1):
                prec = validate(model, latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger)
            else:
                prec = validate(model_teacher, latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger)
            ...
            if rank == 0:
                state = {
                    ...
                    "best_miou": best_prec,
                    "deterministic_memory": deterministic_memory_centers,
                    "latent_memory": latent_memory_centers ,
                }
                if prec > best_prec:
                    best_prec = prec
                    torch.save(
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            outs = model(image_l, latent_memory, deterministic_memory, x_context_in=image_l, labels_target_in=label_l, labels_context_in=label_l)
 
            

            pred, rep = outs["pred"].mean(0), outs["rep"]
            ...
            _ = model_teacher(image_l, latent_memory, deterministic_memory, x_context_in=image_l, labels_target_in=label_l, labels_context_in=label_l)
            
            unsup_loss = 0 * rep.sum()
            contra_loss = 0 * rep.sum()
            kl_divergence = 0 * rep.sum()
        else:
            if epoch == cfg["trainer"].get("sup_only_epoch", 1):
            ...
            pred_u_teacher = model_teacher(image_u, latent_memory_centers, deterministic_memory_centers, phase_train=False)["pred"]
            forward_times, b_o, d_o, h_o, w_o = pred_u_teacher.size()

            pred_u_teacher = pred_u_teacher.view(forward_times * b_o, d_o, h_o, w_o)
            ...
            pred_u_teacher = pred_u_teacher.view(forward_times, b_o, d_o, h, w).mean(0)
            pred_u_teacher_uncer = torch.sum(-1.0 * pred_u_teacher * torch.log(pred_u_teacher), dim=1)
            
            logits_u_aug, label_u_aug = torch.max(pred_u_teacher, dim=1)

            uncer_thresh = 0.4
            pred_thresh = 0.8
            pred_u_teacher_mask = (pred_u_teacher_uncer.le(uncer_thresh)).bool() * (logits_u_aug.ge(pred_thresh)).bool()
            pred_teacher_mask = torch.zeros(label_l.size()).bool().cuda(model.device_ids[0])
            ...
            label_all = torch.cat((label_l, label_u_aug))
            target_mask = torch.cat((pred_teacher_mask, pred_u_teacher_mask))
            
            '''
            target_mask = ~target_mask 
            label_all[target_mask] = 255
            '''
            
            outs = model(image_all, latent_memory, deterministic_memory, x_context_in=image_l, labels_target_in=label_all, labels_context_in=label_l)
       
            pred_all, rep_all = outs["pred"].mean(0), outs["rep"]
            ...
            mean_c_all = outs['mean_c']
            sigma_c_all = outs['sigma_c']
            mean_t_all = outs['mean_t']
            sigma_t_all = outs['sigma_t']
         
        
            
            D = sigma_t_all.size(-1)

            mean_res = mean_c_all-mean_t_all 
            

            kl_divergence = 0.5 * torch.sum(torch.log(sigma_c_all/sigma_t_all), dim=1) - 0.5 * D + \
            0.5 * torch.sum(1.0/sigma_c_all * sigma_t_all, dim=1) + torch.diagonal(0.5 * torch.mm((mean_res*(1.0/sigma_c_all)), torch.transpose(mean_res, 0, 1)))
             
            # teacher forward
            model_teacher.train()
            with torch.no_grad(): 
                out_t = model_teacher(image_all, latent_memory_centers, deterministic_memory_centers, phase_train=False)


                pred_all_teacher, rep_all_teacher = out_t["pred"], out_t["rep"]
                prob_all_teacher = F.softmax(pred_all_teacher, dim=2)
                prob_all_teacher = prob_all_teacher.mean(0)
                pred_all_teacher = pred_all_teacher.mean(0)
        ...
        loss = sup_loss + unsup_loss + contra_loss + 0.005 * kl_divergence.mean()
...
def validate(
    ...
    for step, batch in enumerate(data_loader):
        ...
        with torch.no_grad(): 
            outs = model(images, latent_memory, deterministic_memory, phase_train=False)

        # get the output produced by model_teacher
        output = outs["pred"].mean(0)
