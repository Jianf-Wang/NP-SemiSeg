        ...
        if "cityscapes" in cfg["dataset"].get("type", "pascal"):
            if epoch % 10 == 0 or epoch > (cfg_trainer["epochs"]-50):
                if cfg_trainer.get("evaluate_student", True):
                    prec_stu = validate_citys(model, latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger, cfg)
                else:
                    prec_stu =-1000.0
                prec_tea = validate_citys(model_teacher, latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger, cfg)
                prec = prec_tea
            else:
                prec_stu = -1000.0
                prec_tea = -1000.0
            ...
            if cfg_trainer.get("evaluate_student", True):
                prec_stu = validate(model, latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger, cfg)
            else:
                prec_stu = -1000.0
            prec_tea = validate(model_teacher,latent_memory_centers, deterministic_memory_centers, val_loader, epoch, logger, cfg)
            prec = prec_tea
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            # forward 
            pred, aux, _, _, _, _, _, _ = model(image_l, latent_memory, deterministic_memory, context_in=image_l, labels_target_in=label_l, labels_context_in=label_l)
            ...
            kl_divergence = torch.tensor(0.0).cuda()
            ...
            with torch.no_grad():
                ...
                pred_u, _ = model_teacher(image_u_weak.detach(), latent_memory_centers, deterministic_memory_centers, phase_train=False)

                pred_u = F.softmax(pred_u, dim=2)
                pred_u = pred_u.mean(0)
                # obtain pseudos
            ...
            if flag_extra_weak: 
                ...
                image_all = torch.cat((image_l, image_u_weak, image_u_aug))
                label_all = torch.cat((label_l, label_u_aug, label_u_aug))

                pred_all, aux_all, mean_t_all, sigma_t_all, mean_c_all, sigma_c_all, deterministic_memory, latent_memory = model(image_all, latent_memory, deterministic_memory, x_context_in=image_l, labels_target_in=label_all, labels_context_in=label_l)
                pred_all = pred_all.mean(0)
        ...
        loss = sup_loss + unsup_loss + 0.005 * kl_divergence
...
def validate(
    ...
    for step, batch in enumerate(data_loader):
        ...
        with torch.no_grad(): 
            output, _ = model(images, latent_memory, deterministic_memory, phase_train=False)
    ...
    for step, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            while row < h:
                col = 0
                while col < w: 
                    pred, _ = model(images[:, :, row: min(h, row + crop_size), col: min(w, col + crop_size)], latent_memory, deterministic_memory, phase_train=False)
                    pred = pred.softmax(dim=2)
                    pred = pred.mean(0)

                    final[:, :, row: min(h, row + crop_size), col: min(w, col + crop_size)] += pred
