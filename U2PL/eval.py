...
def main():
    ...
    latent_memory = checkpoint['latent_memory']
    deterministic_memory = checkpoint['deterministic_memory']
...
@torch.no_grad()
def net_process(model, image, latent_memory, deterministic_memory):
    b, c, h, w = image.shape
    # num_classes = cfg['net']['num_classes']
    # output_all = torch.zeros((6, b, num_classes, h, w)).cuda()
    input = image.cuda()
    output = model(input, latent_memory, deterministic_memory, phase_train=False,  forward_times=20)["pred"]
 
    output = F.softmax(output, dim=2).mean(0)
...
def scale_crop_process(model, latent_memory, deterministic_memory, image, classes, crop_h, crop_w, h, w, stride_rate=2 / 3):
        ...
        for index_w in range(0, grid_w):
            ...
            with torch.no_grad():
                prediction_crop[:, :, s_h:e_h, s_w:e_w] += net_process(
                    model, image_crop, latent_memory, deterministic_memory 
                )
...
def scale_whole_process(model, latent_memory, deterministic_memory, image, h, w):
    with torch.no_grad():
        prediction = net_process(model, image, latent_memory, deterministic_memory)
    prediction = F.interpolate(
...
def validate_city(
    ...
    for i, (input_pth, label_path) in enumerate(data_list):
        ...
        for scale in scales:
            ...
            prediction += scale_crop_process(
                model, latent_memory, deterministic_memory, image_scale, classes, crop_h, crop_w, h, w
            )

        prediction_mask = prediction.eq(0.0).bool() 
        prediction[prediction_mask] = 0.00001
   ...
   # logger.info(" * PAvPU {:.4f}".format(np.mean(papu_all)))
    logger.info("<<<<<<<<<<<<<<<<< End Crop Evaluation <<<<<<<<<<<<<<<<<")


def valiadte_whole(
    model, latent_memory, deterministic_memory, classes, data_list, mean, std, scales, gray_folder, color_folder
):
    ...
    for i, (input_pth, label_path) in enumerate(data_list):
        ...
        for scale in scales:
            ...
            prediction += scale_whole_process(model, latent_memory, deterministic_memory, image_scale, h, w)
